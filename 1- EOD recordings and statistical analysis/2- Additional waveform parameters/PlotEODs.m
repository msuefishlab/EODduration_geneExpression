...
for i = 1:length(averaged_eods)
    measurement_data(i) = standard_eod_measurement(averaged_eods(i).wave, averaged_eods(i).sampRate, averaged_eods(i).sample_name, averaged_eods(i).period,averaged_eods(i).treatment,averaged_eods(i).treatdate,averaged_eods(i).individual);
...
outputfilename = fullfile(root,'output_data/02_EOD_Plots','measurement_data.csv'); % Assuming full path is provided in the TSV
